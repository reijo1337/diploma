...
class SkinDetector(object):
    ...
    # function to process the image and segment the skin using the HSV and YCbCr colorspaces, followed by the
    # Watershed algorithm
    def find_skin(self):
        self.__color_segmentation()
        return self.__region_based_segmentation()

    # Apply a threshold to an HSV and YCbCr images, the used values were based on current research papers along with
    # some empirical tests and visual evaluation
    ...
    def __region_based_segmentation(self):
        ...
        return cv2.bitwise_and(self.image, self.image, mask=image_mask)
